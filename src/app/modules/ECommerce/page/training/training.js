...
export function TrainingPage() {
  ...
  const [trainings, retrieveTraining] = useState([]);
  const [deleteModalIsOpen, setDeleteModal] = useState(false);
  const [updateModal, setUpdateModal] = useState(false);
  const [trainingId, setSelectTraining] = useState("");
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/api/auth/all')
      .then(response => {
        retrieveTraining(response.data.training);
      })
      .catch(err => {
        toastForFail("Cannot retrieve training");
      });
  }, []);

  const deleteTraining = () => {
    axios.delete("http://localhost:3001/api/auth/" + trainingId)
      .then(
        (response) => {
  ...
  var _rows = trainings.map(training => {
    var name = training.fullname;
    var age = training.age;
    var address = training.address;
    var contact = training.contact;
    var emContact = training.emContact;
    var email = training.email;
    var date = moment(training.createdAt).format('DD/MM/YYYY');
    var createdAt = date;
    var action = <div className="justify-content-start text-start">
        <button className="action btn btn-primary fas fa-solid fa-book"
          onClick={() => {
            setSelectTraining(training._id);
            setUpdateModalOpen();
          }} ></button>
          ...
          <div className="col">
            ...
            <div className="AddTraining">
              ...
              >
                <Modal.Header closeButton >
                  <div className="w-100">
                    <h1 className="text-center">Add training</h1>
                  </div>
                </Modal.Header>
                 <AddTraining closeAddTraining={setModalIsOpenToFalse} />
              </Modal>
            </div>
            ...
            <div className="updateTraining">
              ...
              >
                <Modal.Header closeButton >
                  <div className="w-100">
                    <h1 className="text-center">Update Ustraininger</h1>
                  </div>
                </Modal.Header>
                <UpdateTraining closeUpdateTrainingModal={setUpdateModalClose} id={trainingId} />
              </Modal>
            </div>
            ...
            <div className="deleteVehicle">
              ...
              >
                ...
                <Modal.Body ><strong>Are you sure you want to delete this training?</strong></Modal.Body>
                <Modal.Footer>
